...
async function createMarket(data) {
  ...
  await harperdb.insert("pylomarket", "markets", [market]);
...
async function getMarket(marketId) {
  ...
  const yesOrders = ordersList.filter(
    (o) => o.outcome === "YES" && o.status === "open"
  );
  const noOrders = ordersList.filter(
...
async function resolveMarket(data) {
  ...
  await harperdb.update("pylomarket", "markets", [market]);
