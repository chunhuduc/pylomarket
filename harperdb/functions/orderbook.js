...
async function placeOrder(data) {
  ...
  if (balances.length === 0 || balances[0].balance < quantity * price) {
  ...
  await harperdb.insert("orders", [order]);
...
async function cancelOrder(data) {
  ...
  const orders = await harperdb.searchByHash("orders", [orderId]);
  ...
  await harperdb.update("orders", [order]);
  ...
  for (const buyOrder of buyOrders) {
    for (const sellOrder of sellOrders) {
      if (buyOrder.price >= sellOrder.price) {
        ...
        await harperdb.update("orders", [buyOrder, sellOrder]);
...
async function executeTrade(buyOrder, sellOrder, quantity, price, marketId) {
  ...
  await harperdb.insert("trades", [trade]);
